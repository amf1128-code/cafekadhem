...
- **Status change yes → no on a paid RSVP**: allowed. The RSVP row keeps `payment_status='paid'` and `ticket_token`, so the QR remains valid at the door (a guest who declined and changed their mind day-of can still walk in). The admin Tickets page shows every row with payment activity regardless of RSVP status, so the host sees "Alice has a paid ticket but declined" and can `mark_rsvp_unpaid` / refund manually if they want to invalidate. Plus-ones are deleted automatically by the existing `sync_plus_one_status` trigger when parent leaves yes/waitlisted. *(History: an earlier policy blocked this transition at the server; reverted in migration 046 after the user reframed the model — paid tickets are durable receipts, RSVP status is the attendance signal, and the two are independent.)*
