-- ============================================================
-- 055: Make upsert_guest RECOMPUTE notification_preference from the
-- filled channels on every upsert, instead of only inferring when the
-- existing value is NULL.
--
-- Problem this fixes (USER_FLOWS_SPEC.md §7.1):
--   Migration 042 used COALESCE(v_pref, notification_preference,
--   v_inferred_pref). The middle term meant: once a guest had ANY
--   preference, inference was ignored forever. So a guest who first
--   RSVP'd with phone-only got 'sms', and later adding an email never
--   flipped them to 'both' — they silently stopped getting the email
--   confirmations they'd expect.
--
-- New rule — "fan out to whatever they give us":
--   * notification_preference = 'none'  -> stays 'none'. Unsubscribe is
--     sticky and consent-bound (record_unsubscribe, migration 040). It
--     is NOT a channel preference and must never be recomputed away.
--   * explicit p_fields.notification_preference (v_pref) -> respected.
--     No current caller sends this; kept for a future Settings UI.
--   * otherwise -> v_inferred_pref, recomputed from the filled channels:
--       email + phone -> 'both'
--       phone only    -> 'sms'
--       email only    -> 'email'
--     v_inferred_pref is guaranteed non-null because the function still
--     requires email or phone (the guard below).
--
-- Caveat (intentional): an inferred 'email' and an admin-set 'email'
-- are stored identically — there is no provenance column — so a manual
-- admin override (e.g. "email-only despite having a phone") will be
-- recomputed to 'both' on that guest's next upsert. This is the desired
-- "fan out to everything" behavior and supersedes the admin-split note
-- in migration 040.
--
-- All other behavior is identical to migration 042.
-- ============================================================
...
BEGIN
  ...
  -- Inferred preference based on which channels were submitted. Always
  -- non-null here: the guard above ensures at least one of email/phone.
  ...
  IF v_email_match IS NOT NULL
    ...
    UPDATE guests SET
      ...
      notification_preference = CASE
                                  WHEN notification_preference = 'none' THEN 'none'
                                  WHEN v_pref IS NOT NULL THEN v_pref
                                  ELSE v_inferred_pref
                                END,
      updated_at              = now()
    WHERE id = v_keep
  ...
  ELSE
    INSERT INTO guests (
      ...
      COALESCE(v_pref, v_inferred_pref)
    )
    RETURNING * INTO v_guest;
  END IF;
